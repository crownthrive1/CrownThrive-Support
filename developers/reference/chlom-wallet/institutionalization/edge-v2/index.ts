...
function errorCode(error: unknown) {
  ...
  return /^[A-Za-z0-9_:-]{1,180}$/.test(value)
    ? value
    : "institutionalization_v2_ingest_failed";
}
...
async function rpc(name: string, body: Record<string, unknown>) {
  ...
  if (!response.ok) {
    try {
      ...
      if (/^[A-Za-z0-9_:-]{1,180}$/.test(message)) throw new Error(message);
    } catch (error) {
...
Deno.serve(async (request: Request) => {
  if (request.method === "GET") {
    return json({
      ...
      authentication: "github_oidc_rs256",
      audience: "chlom-wallet-institutionalization-v2",
  ...
  try {
    ...
    const verifiedToken = await verifyGithubOidcJwt(token);
    const verifiedClaims = await verifyRepositorySnapshot({
    ...
    return json({
      ok: false,
      state: "HOLD_INSTITUTIONALIZATION_V2_INGEST",
